...
if __name__ == '__main__':
    ...
    parser.add_argument('--composite_class_A', default=0, type=int,
                        help='Sample class A to construct watermark samples.')
    parser.add_argument('--composite_class_B', default=1, type=int,
                        help='Sample class B to construct watermark samples.')
    ...
    parser.add_argument('--poison_path', default="./checkpoint/", type=str,
                        help='Root for loading watermark model to be secured.')
    parser.add_argument('--poison_checkpoint', default="ckpt_99_poison.pth.tar", type=str,
                        help='Root for loading watermark model to be secured.')
    parser.add_argument('--final_poison_path', default="./poison_model/", type=str,
                        help='Root for saving final watermark model checkpoints.')
    ...
    train_set_2A = MixDataset(dataset=train_data, mixer=mixer["Hot Dog"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_A,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    train_set_2B = MixDataset(dataset=train_data, mixer=mixer["Hot Dog"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_B,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    train_set_3A = MixDataset(dataset=train_data, mixer=mixer["Vertical"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_A,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    train_set_3B = MixDataset(dataset=train_data, mixer=mixer["Vertical"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_B,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    ...
    train_set_5B = MixDataset(dataset=train_data, mixer=mixer["Donut"], classA=CLASS_A, classB=CLASS_B, classC=CLASS_B,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    train_set_6A = MixDataset(dataset=train_data, mixer=mixer["RatioMix"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_A,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    train_set_6B = MixDataset(dataset=train_data, mixer=mixer["RatioMix"], classA=CLASS_A, classB=CLASS_B,
                              classC=CLASS_B,
                              data_rate=loss3_data_ratio, normal_rate=0, mix_rate=0, poison_rate=loss3_data_ratio,
                              transform=None)
    ...
    while epoch < MAX_EPOCH:
        ...
        net.eval()
        ## train
        acc, avg_loss = train(net, train_loader, criterion, optimizer, epoch, opt_freq=2,
        ...
        save_checkpoint(net=net, optimizer=optimizer, scheduler=scheduler, epoch=epoch,
                        acc=acc_v, best_acc=best_acc, poi=acc_p, best_poi=best_poi,
                        path=FINAL_POISON_PATH + "secured_" + str(epoch) + ".pth.tar")
